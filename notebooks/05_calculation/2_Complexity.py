...
adj_mat = pd.pivot_table(c_df2[c_df2[f'{ar}_{year_style}_period']==f'{year_start}-{year_end}'].sort_values(by=['ubiquity'], ascending=[False], ignore_index=True), 
                         index=region_corporation, columns=classification, values='mcp').fillna(0)\
    .values
# 隣接区域以外を非表示化
...
fig.savefig(f'{output_dir}adjacency_matrix_{output_condition}.png', 
...
classification_df[classification_df[f'{ar}_{year_style}_period']==f'{year_start}-{year_end}']\
    [['schmoch35', 'reg_num', 'ubiquity', 'tci']].drop_duplicates(keep='first')\
...
ps_df[['node_id', 'label', 'reg_num', 'ubiquity', 'tci', 'schmoch5']].to_csv(f'{output_dir}product_space/{output_condition}_node.tsv', 
            ...
            encoding='utf-8', 
            index=False)
ps_df.drop_duplicates(keep='first', ignore_index=True)\
     .sort_values(by=['ki_1'], ascending=[False], ignore_index=True)
